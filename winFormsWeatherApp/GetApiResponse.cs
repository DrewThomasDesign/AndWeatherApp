...
namespace winFormsWeatherApp
{
    ...
    {
        public static string GetForecast(string url)
        ...
        public static string GetWeatherImage(string url)
        {
            var client = new WebClient();
            var uri = new Uri(url);
            client.DownloadFileCompleted += (sender, e) => Console.WriteLine("Image Download Finished.");
            client.DownloadFileAsync(uri,"Weather.gif");

            // weatherBox.LoadAsync(getApiResponse.GetWeatherImage(txtApiUrl.Text));

            RootObject deserializedBaseUrl = GetSerializedWeatherResponse(url);
            string deserializedUrl = deserializedBaseUrl.hourly_forecast[0].icon_url;
